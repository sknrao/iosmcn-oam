...
check_images(){
export ESRAPP_IMAGE="es-rapp:latest"
}
...
while [ $retcode -ne 0 ]; do
    ...
    retcode=$?
    test_string="Topic 'es-rapp-topic' already exists"
...
envsubst < docker-compose-esrapp.yaml > docker-compose-esrapp_gen.yaml
docker compose -p esrapp -f docker-compose-esrapp_gen.yaml up -d
}
## Main ##
setup_init

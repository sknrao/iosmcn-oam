...
//lint:ignore S117
func xmlToJsonConvNewFormat(fBytevalue *[]byte, xfeh *dataTypesNR.XmlFileEventHeader) ([]byte, error) {
	var newFile dataTypesNR.MeasDataFile
	start := time.Now()
	err := xml.Unmarshal(*fBytevalue, &newFile)
	if err != nil {
	...
	start = time.Now()
	var pmfile dataTypesNR.PMJsonFile
	...
	pmfile.Event.Perf3GppFields.MeasDataCollection.MeasuredEntityDn = newFile.FileHeader.FileSender.SenderName
	pmfile.Event.Perf3GppFields.MeasDataCollection.MeasuredEntitySoftwareVersion = "N/A"

	for _, it := range newFile.MeasData.MeasInfo {
		var mili dataTypesNR.MeasInfoList
		...
		for _, jt := range it.MeasValue {
			...
			for _, kt := range jt.R {
				...
				nv := kt.Text
				mr := dataTypesNR.MeasResults{ni, nv}
...
func Convert(inputS3Url, compression, xmlFileEventHeader string) []byte {
	evtData := dataTypesNR.XmlFileEventHeader{}
	...
	b, err := xmlToJsonConvNewFormat(&fileBytes, &evtData)
	return b
}
